...
- **The verification record is a comment on each covered epic**: the truth table plus the evidence summary, posted before closing anything. It's what a future session reads to know verification actually happened. Issues with no epic (audit-filed findings, quick captures) get their record on the milestone's PR — or on the issue itself when there's no PR. Issues without a must-haves block (audit findings) get their checks derived from AC, same as `--auto` does for Demo-less stories.
